...
// The standalone launcher. This is the SAME resolver pair the inline script
// uses, with the flags taken from "$@" instead of baked in, so one launcher
// serves every backend and the harness config carries only a command and its
// arguments.
//
// Installed to a stable, version-independent path (see `--print-launcher` and
// the mcp-shim skill), exactly as `roundhouse launcher-install` does for the
// CLI: the config cannot name a versioned plugin path without breaking on the
// next update, which is the whole reason the resolver existed inline. Moving
// it into a file keeps that property and takes the shell out of the config.
//
// It also removes a class of bug rather than guarding against it: with no TOML
// multiline literal there is no delimiter to collide with a shell quote.
export function buildLauncherScript() {
  return `#!/bin/sh
# Generated by mcp-siding --print-launcher. Resolves the newest installed
# mcp-siding.mjs and a usable node at RUN time, so this file stays correct
# across plugin updates and never needs to name a version.
${RESOLVER_SH}
${NODE_RESOLVER_SH}
exec "$node_bin" "$p" "$@"
`;
...
// The argv a harness registration should carry alongside the launcher path.
// Same builder the inline script uses, so the two registration flavours can
// never drift in which flags they forward.
export function launcherRegistrationArgs(flags) {
  return shimFlagArgs(flags);
}
...
const BOOL_FLAGS = new Set(["selftest", "help", "launch", "no-launch", "print-resolver", "print-shim-script", "print-launcher"]);
...
function run() {
  ...
  if (flags["print-launcher"]) {
    // Takes no backend flags: the launcher is backend-agnostic and reads them
    // from "$@" at run time, so there is nothing here to validate.
    process.stdout.write(buildLauncherScript());
    return 0;
  }
