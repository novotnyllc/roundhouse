...
YAML
    ...
    # shell preflight cannot itself assess them, and a go/no-go gate must not
    # report "ready" for a host it never actually checked. It stays a
    # finding (never silently "ok"), names the controller contract instead
    # of a bare failure, and must not reinterpret the native Windows target
    # as an SSH target.
    u22_codex_status=0
